...
while(true) // Всегда работает
{
    try // Обработка исключения
    {
        ...
        {
            if (num3str.Length >= 3 && Regex.IsMatch(num3str, pattern)) // Плюс, если длина строки больше или равна 3, то, переходи дальше
            {   
                if (int.Parse(num3str).ToString().Length >= 3) // Если конвертированная строка в целочисленное больше или равно 3, то, переходи дальше
                {
                ...
                else // Иначе длина конвертированной в целочисленной строки меньше 3
                {
                    Console.Write($"Чтобы выйти из программы, введите символ \"q\" в терминале.\nВведёное число {int.Parse(num3str).ToString()} меньше трёх цифр.\n"
                                    +"Введите целое трёхзначное число: ");
                };
            ...
            {
                Console.Write("Чтобы выйти из программы, введите символ \"q\" в терминале.\nТретьей цифры нет! Введите целое трёхзначное число: ");
            }
        }
